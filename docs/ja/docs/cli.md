...
OrniFlight のコマンド ライン インターフェイス (CLI) では、すべての設定に完全にアクセスできます。 OrniFlight コンフィギュレーターの **CLI** タブ経由、または 115200 ボーのシリアル端末経由でアクセスします。
...
| `help` |すべてのコマンドを表示 |
| `status` |システムステータスを表示 |
| `version` |ファームウェアのバージョンを表示 |
| `defaults` |すべての設定をデフォルトにリセットする |
| `save` |設定を保存して再起動 |
| `exit` | CLI を終了します (保存せずに再起動します) |
| `diff` |変更された設定を表示 (構成の共有に役立ちます) |
| `dump` |すべての設定を表示 |
| `dump all` |ハードウェアのデフォルトを含むすべての設定を表示 |
...
`diff` を使用して、デフォルト以外の設定のみをエクスポートします。
