...
Antarmuka Baris Perintah OrniFlight (CLI) menyediakan akses penuh ke semua pengaturan. Akses melalui tab **CLI** di OrniFlight Configurator, atau melalui terminal serial pada 115200 baud.
...
| `version` | Tampilkan versi firmware |
| `defaults` | Reset semua pengaturan ke default |
| `save` | Simpan pengaturan dan reboot |
| `exit` | Keluar dari CLI (reboot tanpa menyimpan) |
| `diff` | Tampilkan pengaturan yang diubah (berguna untuk berbagi konfigurasi) |
| `dump` | Tampilkan semua pengaturan |
| `dump all` | Tampilkan semua pengaturan termasuk default perangkat keras |
...
## ONDAS Pengaturan (apiVersi 1.46+)
...
## Sudut Pemasangan Servo (apiVersion 1.46+)
...
## Berbagi Konfigurasi Anda

Gunakan `diff` untuk hanya mengekspor pengaturan non-default:
