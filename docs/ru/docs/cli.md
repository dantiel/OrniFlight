...
Интерфейс командной строки OrniFlight (CLI) обеспечивает полный доступ ко всем настройкам. Доступ осуществляется через вкладку **CLI** в конфигураторе OrniFlight или через последовательный терминал на скорости 115200 бод.
...
| `version` | Показать версию прошивки |
| `defaults` | Сбросить все настройки к значениям по умолчанию |
| `save` | Сохраните настройки и перезагрузитесь |
| `exit` | Выход CLI (перезагрузка без сохранения) |
| `diff` | Показать измененные настройки (полезно для обмена конфигурациями) |
| `dump` | Показать все настройки |
| `dump all` | Показать все настройки, включая настройки оборудования по умолчанию |
...
## ONDAS Настройки (apiVersion 1.46+)
...
## Угол крепления сервопривода (apiVersion 1.46+)
...
## Публикация вашей конфигурации

Используйте `diff` для экспорта только настроек, отличных от настроек по умолчанию:
